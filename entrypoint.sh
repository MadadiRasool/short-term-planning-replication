#!/bin/bash
python --version


# python run.py
echo -e 'Start Get Date'
source activate short-term-planning-replication & python scripts/01-construct-estimation-data.py
echo -e 'End Get Date'
source activate short-term-planning-replication & python scripts/02-construct-fortran-models.py

# python print("Start Create Fortran Models")
# python scripts/02-construct-fortran-models.py

# python print("Start Create BaseLine Models")
# python scripts/03-estimate-baseline-models.py --model canonical_NK --nprocs 4
